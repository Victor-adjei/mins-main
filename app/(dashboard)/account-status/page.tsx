...
export default function AccountStatusPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Account Status</h1>
        <p className="text-slate-500 font-medium mt-1">Manage lifecycle states for member accounts.</p>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 sticky top-24">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-slate-400 uppercase tracking-widest mb-3">Status Name</label>
                <input spellCheck={false} 
                  ...
                  placeholder="e.g. Active, Dormant, Closed"
                />
              </div>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
            <div className="p-8 border-b border-slate-50 bg-slate-50/20">
               <h3 className="text-lg font-bold text-slate-900 uppercase tracking-tight flex items-center">
                 <Settings2 className="w-5 h-5 mr-3 text-emerald-500" />
            ...
            <div className="divide-y divide-slate-50">
              {statuses.length > 0 ? statuses.map((status) => (
                <div key={status.account_status_number} className="p-6 flex items-center justify-between hover:bg-slate-50/50 transition-colors group">
                  <div className="flex items-center space-x-4">
                    <div className="w-10 h-10 rounded-xl bg-rose-50 flex items-center justify-center text-rose-600 shadow-sm border border-rose-100">
                      <CheckCircle2 className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-900 leading-none">{status.account_status_name}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5">ID: #{status.account_status_number}</p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center py-24 px-8 text-center">
                   ...
                   <h4 className="text-sm font-bold text-slate-900 underline decoration-rose-200 decoration-2 underline-offset-4">No account statuses defined</h4>
                </div>
              )}
            </div>
          </div>
