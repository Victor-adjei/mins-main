...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    if (modalAccountNumber.length === 10) {
      handleModalLookup(modalAccountNumber);
    } else {
      setModalAccountDetails(null);
    }
  }, [modalAccountNumber]);

  async function handleModalLookup(accNum: string) {
    setFetchingModalAccount(true);
    try {
      const res = await fetch(`/api/accounts?accountNumber=${accNum}`);
      const data = await res.json();
      if (Array.isArray(data) && data.length > 0) {
        setModalAccountDetails(data[0]);
      } else {
        setModalAccountDetails(null);
      }
    } catch (error) {
  ...
  return (
    <div className="bg-[#f8fafc] min-h-screen">
                  ...
                  <button 
                    onClick={() => {
                      if (/^\d{10}$/.test(searchQuery)) {
                        setModalAccountNumber(searchQuery);
                      } else {
                    ...
                    className="px-6 py-3 bg-[#0066cc] text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-blue-700 transition-all shadow-lg active:scale-95 flex items-center space-x-3 w-full sm:w-auto justify-center"
                  >
                          ...
                          <tr key={tx.transaction_id} className="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td className="px-8 py-6 text-right">
                              <div className="flex items-center justify-end space-x-3">
                                <a 
                                  href={`/api/pdf/transactions?accountNumber=${tx.account_number}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  title="Download Statement"
                                  className="p-3 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all shadow-sm border border-transparent hover:border-emerald-100"
                                >
                                ...
                                {canModify ? (
                                  ...
                                  <span title="Locked (12h passed)" className="p-3">
      ...
      {showDownloadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in duration-300">
            <div className="w-16 h-16 bg-blue-100 text-[#0066cc] rounded-2xl flex items-center justify-center mb-6">
              <FileDown className="w-8 h-8" />
            </div>
            <h3 className="text-2xl font-black text-slate-900 tracking-tight">Download Statement</h3>
            <p className="text-slate-500 font-medium mt-2 leading-relaxed text-sm">
              Enter a 10-digit account number to verify the member and generate a PDF statement.
            </p>
            
            <div className="mt-8 space-y-6">
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest ml-1 mb-2">Account Number</label>
                <div className="relative">
                  <input 
                    type="text"
                    maxLength={10}
                    ...
                    placeholder="Enter 10-digit A/C"
                    className="w-full px-5 py-5 bg-slate-50 border-none focus:ring-2 focus:ring-blue-500/20 rounded-2xl text-xl font-black tracking-widest text-slate-900 outline-none transition-all placeholder:tracking-normal placeholder:text-sm"
                  />
                  {fetchingModalAccount && (
                    <div className="absolute right-4 top-1/2 -translate-y-1/2">
                      <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />
                    </div>
                  )}
              ...
              {/* Modal Account Owner Preview Card */}
              {modalAccountNumber.length === 10 && (
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100 space-y-2 animate-in slide-in-from-top-2">
                  {modalAccountDetails ? (
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">Member Name</span>
                        <span className="text-xs font-black text-slate-800">{modalAccountDetails.first_name} {modalAccountDetails.surname}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">A/C Rank</span>
                        <span className="text-xs font-black text-slate-800">Rank {modalAccountDetails.account_rank}</span>
                      </div>
                      <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                        <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">Balance</span>
                        <span className="text-sm font-black text-emerald-600">GHS {parseFloat(modalAccountDetails.balance.toString()).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      </div>
                    </>
                  ) : (
                    !fetchingModalAccount && (
                      <div className="text-center py-2 text-rose-600 text-xs font-bold">
            ...
            <div className="mt-10 grid grid-cols-1 gap-4">
              <a 
                href={modalAccountDetails ? `/api/pdf/transactions?accountNumber=${modalAccountNumber}` : undefined}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => {
                  if (modalAccountDetails) {
                    setShowDownloadModal(false);
                  }
                }}
                className={cn(
                  "py-5 bg-slate-900 text-white rounded-2xl font-black text-center shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center space-x-2 cursor-pointer",
                  !modalAccountDetails && "opacity-50 pointer-events-none cursor-not-allowed"
                )}
              >
              ...
              <button 
                ...
                className="py-4 text-slate-400 font-bold hover:text-slate-600 transition-all text-sm"
              >
                Close Dialog
              </button>
            </div>
